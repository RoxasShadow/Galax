...
class User
  ...
  property :email,            String, unique: true, length: 5..40, format: :email_address  
  property :address,          String  # address = $street $cap $city $province (ex.: Via Ruby 80142, Napoli NA)
  ...
  before :save do
    self.address = Rack::Utils.escape_html self.address
  end

  def street
    self.address.split(/[0-9]+/)[0]
  end

  def city
    self.address.split(/[0-9]+/)[1]
  end
  ...
  def find_people
    [].tap { |people|
      User.all.each { |user|
        ...
        people << user if user.city == self.city
      }
    }
  end
  ...
  class << self
    ...
    def get_by_group(permission_level)
      return nil unless User.levels.values.include? permission_level.to_i
      User.all permission_level: permission_level.to_i
    end
